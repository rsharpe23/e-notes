...
Пример простого правила для сборки программы из 2х исходников:
 ...
 hello: file.c file2.c
     gcc file.c file2.c -o hello

При вызове make hello, утилита попытается получить цель hello (исполняемый файл), 
которая завист от 2х файлов file.c file2.c. Если цели нет, то будет выполнена команда 
gcc file.c file2.c -o hello. Если цель есть но одна из зависимостей изменена, то
команда также будет выполнена и все файлы перекомпилируется заново.
...
Чтобы каждый раз не перекомпилировать все файлы, сборку обычно разделяют:
 ...
 hello file.o file2.o
     gcc file.o file2.o -o hello
...
В примере выше, при вызове make hello, утилита точно также попытается получить цель hello,
...
Целью может быть не только создаваемый файл но и фиктивная цель (таск).
...
Указать фиктивные цели можно с помощью директивы .PHONY
 ...
 all hello                             # Цель зависит от наличия файла hello, т.е.
                                       # если его нет, то сначала выполнится цель hello
 file.o file.c
     gcc -c file.c -o file.o
 ...
 install:
     install ./hello /usr/local/bin    # Команда install cкопирует файл hello 
