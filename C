...
Компиляция файлов выполняется с помощью команды gcc. После компиляции создается исполняемый
...
Для обращения к макросу приеняется конструкция: макрос(аргумент1, аргументN)
 ...
 // При работе с многострочным кодом нужно блок кода.
 // Макрос swap является обобщенным.
