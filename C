...
Структура может иметь вложенную структуру:
 ...
 struct phone { char* name; int price; };
 struct person { char* name; int age; struct phone ph; };

 // Вложенную стрктуру можно инициализировать либо так, 
 // либо через каждое поле по отдельности

 struct person tom = { "Tom", 38, { "Motorola", 810 } };
...
а уже после задать полноценное определение.
 ...
 struct phone;  // Структура неполного типа
...
не получится обращаться и ее размер будет неизвестен.
...
изменять еще и адрес, нужно чтобы указатель на константную структуру сам был константным.
...
Пример создания массива структур с перебором в цикле: 
...
Структуры и функции: https://metanit.com/c/tutorial/6.5.php
