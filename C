...
Указатель на указатель может заменять указатель на массив: 
...
Массиву в параметрах можно установить минимальную длину с пом. оператора static:
 void myFunc(int arr[static 4]) { ... }  // ф-ция ожидает массив минимум с 4 эл-тами
...
Синтаксис объявления напоминает то, как объявляется указатель на массив (тоже в скобках).
...
параметра типа int и возвращают double:
 ...
 double (*actions)(int, int) = { add, sub, mul };
 double res = actions[0](10, 5);

С помощью оператора typedef можно задать псевдоним (тип) ф-ции. Такие псевдонимы упрощают
применение указателей на ф-ции: typedef возвращаемый_тип (псевдоним)(типы_параметров)

 typedef void (message)(void);

 // Здесь msg - это указатель на ф-цию типа message. Тип message - 
 // это псевдоним ф-ции, которая ничего не принимает и не возвращает
