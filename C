...
Выравнивание поля позволяет ему размещаться только по тем адресам, которые кратные размеру его типа....

Выравнивание примитивного типа позволяет ему размещаться только по тем адресам, которые кратны его размеру. Например переменная с типом int может размещаться через каждые 4 байта, переменная с типом long или указатель - через каждые 8 байт, а переменная с типом char - вообще по любому адресу.

Про выравнивание: https://www.youtube.com/watch?v=qrxxRbz8nAU

-----------------

Размер структуры равен сумме размеров ее полей с учетом выравнивания и всегда кратен 
размеру наибольшего поля. Например, размер структуры person будет равен не 12 байт 
(char* + int), а 16, т.к. поле с типом int подвергнется выравниванию.

Выравнивание добавляет после каждого поля некоторый отступ, который равен разнице между размером наибольшего поля и размером текущего поля.

Чтобы оптимизировать размер структуры, поля нужно определять по размеру, 
от большего к меньшему.

Адрес структуры - это адрес ее первого поля. Поля структуры размещаются в памяти в том порядке, в котором они заданы при определении, но опять же, с учетом выравнивания.

-----------------

Выравнивание добавляет после каждого поля некоторый отступ, который равен разнице между размером наибольшего поля и размером текущего поля. Это необходимо для того, чтобы все поля имели одинаковую кратность. 
