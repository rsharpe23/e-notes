...
Компиляция файлов выполняется командой gcc. После компиляции создается исполняемый
файл a.out. Имя выводимого файла можно переопределить с помощью флага -o:

 gcc file.c               -->  создастся a.out
 gcc file.c -o myapp.out  -->  создастся myapp.out
 gcc file.c file2.с       -->  компиляция нескольких файлов
...
нужно задать флаг -pedantic:

 gcc -std=c11 file.c
 gcc -std=c17 -Wall -pedantic file.c & ./a.out  -->  команды разделются через &

Флаг -S позволяет сгенерировать файл с ассемблерным кодом (GAS):

 gcc -S file.c             -->  создастся file.s
 gcc -S file.c -o myasm.s  -->  создастся myasm.s

Флаг -E позволяют выполнить только препроцессор. Чтобу указать препроцессору не выводить
маркеры строк, нужно задать флаг -P.

 gcc -E -P file.c -o newfile.c 

Флаг -c позволяет сгенерировать объектный файл.

 gcc -c file.c file2.c  -->  создадутся file.o и file2.o
 gcc file.o file2.o     -->  компиляция программы из объектных файлов
 gcc file.c file2.o     -->  компиляция из одного c-файла и одного объектного файла

Флаг -shared позволяет создать динамическую библиотеку из объектных файлов:

 gcc -shared file2.o -o lib.so  -->  создание динамической библиотеки
 gcc file.o lib.so              -->  компиляция программы

При запуске программы, которая зависит от динамической библиотеки, необходимо также 
указать путь, по которому та распологается. По умолчанию программа ищет все зависимости 
в системном каталоге (в Linux это /lib):

 export LD_LIBRARY_PATH=.  -->  точка указывает, что динамическая библиотека 
 ./a.out                        находится в текущем каталоге

При создании динамической библиотеки ее объектные файлы дожлны быть сгенерированы 
с флагом -fPIC. Это просто мера безопасности для создания кода, независимого от позиции 
(см. https://metanit.com/c/tutorial/10.1.php)

Флаг -static позволяет поместить все зависимости в исполняемый файл 
(исполняемый файл будет весить больше):

 gcc -static file.c

Флаг -I позволяет указать путь к пользовательским заголовочным файлам.
Путь может быть как абсолютным, так и относительным.

 gcc -I/path/to/my/headers file.c  -->  абсолютный путь
 gcc -Ipath/to/my/headers file2.c  -->  относительный путь
...
Пример считывания строки:
 ...
 // поэтому здесь не используем амперсанд

 // Ширина поля 10 означает, что считываются только первые 10 символов.
 // Но при этом, когда вводимая строка превышает ширину поля, 
 // то все последующие scanf() уже не будут работать.
...
перевода строки '\n' (т.е. пока пользователь не нажмет на Enter). Здесь, возможно,
придется дополнительно вводить разделить (пробел) перед спецификатором,
иначе может получится таже проблема, что и с шириной поля.
...
Файл main.c
 ...
 extern char msg[];
 int add(int, int);  // прототим тоже можно указывать с extern
...
вставка каких-то кусков текста или использование определенного текста в зависимости 
...
Она вставляет текст из какого-то заголовочного файла. Заголовочные файлы содержат
...
Для обращения к макросу приеняется конструкция: макрос(аргумент1, аргументN)
 ...
 // При работе с многострочным кодом, его нужно заключать в блок кода.
 // Здесь макрос swap является обобщенным.
 ...
 // При работе с однострочным выражением, его нужно 
...
Ф-ции для проверки символов (ctype.h):

 * isspace() - проверяет, является ли символ пробелом 
   (пробелами также считются символы: \n, \r, \t);

 * isalnum() - проверяет, явояется ли символ цифрой или алфавитным 
   символом (антипод isspace);

 * isdigit() - проверяет, является ли символ цифрой;
 * isxdigit() - проверяет, является ли символ шестнадцатиричный цифрой;
 * isalpha() - проверяет, является ли символ алфавитным;
 * iscntrl() - проверяет, является ли символ управляющим (\0, \t, и т.д.);
 * isprint() - проверяет, является ли символ печатным (антипод iscntrl)
 * isgraph() - проверяет, является ли символ графическим (который можно увидить)
 * ispunct() - проверяет, является ли символ пунктуационным (точка, запятая и т.д.)
 * islower() - проверяет, представлен ли алфавитный символ в нижнем регистре;
 * isupper() - проверяет, представлен ли алфавитный символ в верхнем регистре;

Все эти ф-ции в случае успеха возвращают какое-то ненулевое число, иначе 0.
Также в этом файле есть ф-ции tolower() и toupper() для перевода символов нужный регистр.
...
Подобным образом можно создавать и обобщенные структуры:
 ...
 person(char) tom = { .id = "cajks234s", .name = "Tom" };
 person(int) bob = { .id = 1234, .name = "Bob" };
...
Первый параметр _Generic - это значение, тип которого будет сопоставляться с ключами из
второго параметра. Второй параметр это коллекция ключ/значение, где ключи - это типы, 
а значения - ф-ции, которые им соответствуют.

Для работы с unicode-символами используется тип wchar_t (wide char), который определен 
в заголовочном файле stddef.h. Его размер больше обычного char и зависит от системы 
(32 бита на Linux и 16 бит на Windows).

Значения wchar_t - это символы с префиксом L. Для вывода на консоль 
строки wchar_t, используется спецификатор %ls:

 wchar_t c = L'A';
 wchar_t str[] = L"Hello";
 printf("%c %ls", c, str);

Помимо этого, для работы с UTF-8/16/32 можно использовать типы char8_t, char16_t 
и char32_t, которые определены в заголовочном файле uchar.h. Значения этих типов также
помечаются определенными префиксами:

 char8_t a = u8'N';
 char16_t b = u'Ы';
 char32_t c = U'Ю';

 // Вместо символов можно использовать числовые коды 
 // в соответсвующей кодировке
 
 char16_t str[] = u"\u043F\u0440\u0438\u0432\u0435\u0442";

 // Для вывода строки char16_t используется спецификатор %ls, 
 // а для вывода символа char16_t - спецификатор %lc

 printf("%lc %ls", b, str);  -->  "Ы привет"

В зависимости от архитектуры, размер базовых типов может меняться. Для описания применяемых
размеров используются модели данных (data model). Есть множество моделей, например: LLP64
(4/4/8 - int/long/указатели), применяемая для 64-разрядных систем в Win32 API или LP64
(4/8/8), применяемая для 64-разрядных Unix-подобных систем и пр.

Чтобы уйти от платформенной зависимости можно использовать числовые типы, определенные 
в заголовочном файле stdint.h. Они именуются по такому шаблону [u]int[размер]_t. Например:
int8_t, int16_t, int32_t, int64_t, intptr_t, uint64_t, uintptr_t и т.п.

Для вывода таких типов на консоль можно использовать стандартные спецификаторы или
макросы, определенные в заголовочном файле inttypes.h

 * PRI - для форматированного вывода (с пом. printf(), fprintf() и т.д.);
...
В языке Си чтобы сделать конкатенацию строк, нужно просто расположить их рядом:
 char* str = "hello" "world";  -->  "hello world"

X-макросы - это параметризованные шаблоны. С их помощью можно создавать абстрактные формы
данных и разворачивать их в конкретные формы где-то в коде.
...
X-макросы можно использовать в качестве параметров, а также выносить в отдельные 
заголовочные файлы (см. https://metanit.com/c/tutorial/12.6.php)
 ...
 #define COLOR_CASE(name, value) case name: return value;

 typedef enum {                 // typedef enum { 
     COLORS(COLOR_NAME)         //     red,green,blue 
 } color;                       // } color; 

 int getValue(color c) {        // int getValue(color c) {             
     switch (c) {               //     switch (c) {
         COLORS(COLOR_CASE)     //         case red: return 0xff0000; case green: ...
     }                          //     }
     return 0;                  //     return 0;
 }                              // }

В Си нет встроенной поддержки обработки исключений. Но это можно реализовать самостоятельно 
с помощью ф-ций setjmp() и longjmp(). Подробнее https://metanit.com/c/tutorial/10.2.php

Компиляторы GCC и Clang позволяют встраивать в исходный код ассемблерные вставки.
Для этого используется выражение asm() - внутри скобок которого определяется код на
ассемблере с подстановками данных из Си (см. https://metanit.com/c/tutorial/10.3.php)

В Си можно использовать атрибуты. Они предоставляют метаинформацию для компилятора.
Объявление выглядит так: __attribute__((атрибут1(аргументы), атрибутN))

Атрибуты указываются либо перед, либо после возвращаемого типа ф-ции, типа параметра
или переменной. Они могут быть как с аргументами, так и без.

 __attribute__((myAttr("hello"))) 
 void myFunc() { }

 void __attribute__((myAttr, myAttr2)) myFunc() { }  

Список некоторых атрибутов:

 * noreturn - указывает компилятору, что ф-ция никогда не возвращает управление 
   вызывающей стороне. Это полезно для оптимизации, когда ф-ция, например, 
   завершает программу (вызывая exit()) или генерирует ошибку;

 * deprecated - используется для обозначения устаревших ф-ций или переменных; 
   компилятор будет генерировать предупреждение при использовании таких эл-тов; 
   
 * unused - используется для обозначения параметров, переменных или полей структуры, 
   которые объявлены, но не используются, чтобы подавить предупреждения;

 * packed - указывает компилятору, что поля структуры должны быть упакованы 
   (т.е. без выравнивания). Это может уменьшить размер структуры, но привести к 
   ухудшению производительности при доступе к полям (полезно в embedded разработке);

 * aligned - указывает компилятору минимальное выравнивание для типа. Используется для
   оптимизации доступа к данным, для соблюдения аппаратных требований (некоторые устройства,
   требуют чтобы данные, которыми те обмениваются, были выровнены по конркетным граница), 
   а также для межязыкового взаимодействия (когда код на Си взаимодействует с кодом 
   на других языках или с внешними библиотеками, может потребоваться явное 
   выравнивание данных для совместимости);

 * constructor и destructor - задают глобальный конструктор и деструктор 
   (ф-ции, которые будут выполнятся перед и после main()); 

Не все компиляторы поддерживают атрибуты (GCC и Clang поддерживают). Если переносимость 
кода является критически важной, то нужно использовать условные директивы, чтобы
компилировать код с атрибутами только для поддерживаемых копмиляторов.

Если проект содержит множество файлов, то может возникнуть ситуация, когда один и тот же
заголовочный файл будет подключен несколько раз, что приведет к ошибке компиляции.

Поэтому загаловочные файлы принято определять с доп. проверкой через #ifndef, 
либо указывать в них директиву #pragma once

Директива #pragma once указывает компилятору, что заголовочный файл нужно подключать 
один раз. Однако она может не поддерживаться некоторыми компиляторами.

Определение файла с проверкой через #ifndef:
...
Сначала идет проверка, не определена ли некоторая константа (если она не определена, 
то это значит, что заголовочный файл не подключен). Затем, если константа не определена - 
определяем ее с помощью #define и далее, вплоть до выр-я #endif, помещаем содержимое файла.

Название константы должно быть уникальным на всем множестве файлов проекта. Поэтому обычно 
в качестве названия используется название заголовочного файла, где точка заменяется 
нижним подчеркиванием (user.h  -->  USER_H).

При работе с внешними библиотеками, названия файлов (и соответсвенно констант) могут
дублироваться. В этом случае к названиям констант можно добавлять уникальн. префикс или UUID.

----------------------

УТИЛИТА MAKE

Используется для автоматизации сборки программ из исходников.
Для описания правил сборки применяют Makefile.

Мэйкфайл должен находится в корневой папке проекта. Таких файлов может быть несколько. 
В этом случае при запуске make нужно явно указать необходимый.

 make               -->  выполнит файл с названием Makefile
 make -f Makefile2  

Правила в мэйкфайле задаются следующим образом:

 цель: зависимость1 зависимостьN
     команда

Цель - это создаваемый файл (например исполняемая программа). Зависимости - это файлы, 
от которых зависит цель. Команда - это какая-то компанда из терминала. Отступ перед командой
должен задаваться с помощью табуляции, а не пробелов.

При запуске команды make можно явно указать правило, которое необходимо выполнить. 
Если никаких правил не задано, то утилита выполнит самое первое.

Пример простого правила для сборки программы из 2х исходников:

 myprog: file.c file2.c
     gcc file.c file2.c -o myprog

При вызове make myprog, утилита попытается получить цель myprog (исполняемый файл), 
которая завист от 2х файлов file.c file2.c. Если цели нет, то будет выполнена команда 
gcc file.c file2.c -o myprog. Если цель есть но одна из зависимостей изменена, 
то команда также будет выполнена и все файлы перекомпилируется заново.

Чтобы каждый раз не перекомпилировать все файлы, сборку обычно разделяют:
 ...
 myprog: file.o file2.o
     gcc file.o file2.o -o myprog

Теперь, если изменится один из файлов исходного кода, то при вызове команды make
перекомпилируется только он (и произойдет линковка). Остальные файлы останутся не тронутыми.

В примере выше, при вызове make myprog, утилита точно также попытается получить цель myprog,
которая теперь зависит от file.o и file2.o. Но по умолчанию этих файлов еще нет, поэтому make
выполнит сначала другие цели, чтобы получить file.o и file2.o, а затем вернется к текущей.

Целью может быть не только создаваемый файл но и фиктивная цель (напоминает таск).
Список стандартных фиктивных целей:

 * all - цель по умолчанию - сборка программы (ставится самой первой);
 * clean - очистить каталог от всех скомпилированных файлов;
 * install - установить программу;
 * uninstall - удалить программу;

Указать фиктивные цели можно с помощью директивы .PHONY

 .PHONE: all clean install uinstall

 all: myprog                           # Цель зависит от наличия файла myprog, т.е.
                                       # если его нет, то сначала выполнится цель myprog
 file.o: file.c
     gcc -c file.c -o file.o
 ...
 install:
     install ./myprog /usr/local/bin   # Команда install cкопирует файл myprog 
                                       # в каталог /usr/local/bin
 uninstall:
     rm -rf /usr/local/bin/myprog

Перед сборкой программы желательно очищать все ранее скомпилированные файлы. Иначе, 
если к примеру забыть указать новую зависимость, make просто ничего не станет
собирать (т.к. цель уже существует, а старые зависимости не изменены).

 make clean
 make

Мэйкфайл может иметь переменные. Они объявляются так: НАЗВАНИЕ = строка с пробелами
В переменные обычно выносят все что может повторятся во время сборки: название компилятора,
флаги, директории файлов и пр. Обращаются к переменной так: $(НАЗВАНИЕ)

Кроме определяемых переменных существуют также автоматические:
 $@ - имя цели обрабатываемого правила;
 $< - имя первой зависимости обрабатываемого правила;
 $^ - список всех зависимостей обрабатываемого правила;

Пример разделения сборки с помощью переменных:
