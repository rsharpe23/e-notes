...
быстро, в отлчие от взаимодействия с физическим устройством типа принтера.
...
При считывании данных структуры, сначала считывается числовой код символа в переменную int 
и только потом он передается указателю "с". Это сделано для того, чтобы было корректное
сравнение с EOF поскольку оно может быть любым отрицательным числом. Без этого трюка, 
если бы структура имела поле с отрицательным числом, то оно интерпритировалось 
бы как конец файла и приводило к преждевременному выходу и цикла.
