...
Выравнивание поля с примитивным типом позволяет ему размещаться только по тем адресам, которые кратны размеру типа. Например поля с типом long или указатели смогут размещаться только по адресам, которые делятся на 8 без остатка.
...
т.к. порядок определения полей в ней неоптимизирован. Поля "a" и "c" будут подвержены выравниванию по наибольшему полю "b" и после них появятся доп. оступы в 4 байта.

Отступы добавляются из-за того, что "b" не может располагаться сразу после "a", 
на 4 ячейке, т.к. она не кратна 8.

Чтобы избежать лишних отступов, поля всегда нужно определять в порядке от наибольшего к наименьшему: struct { char* b, int a, int c }; В оптимизированной структуре поля "a" и "с" занимают ячейки 8 и 12 соответсвенно, т.к. они кратны 4 и соответсвенно их не нужно выравнивать. В итоге размер структуры будет ожидаемые 16 байт.
