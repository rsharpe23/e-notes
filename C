...
Для работы с динамической памятью используются ф-ции из заголовочного файла stdlib.h
 ...
 * realloc(*bl, ns) - изменяет размер выделенной памяти до ns байт, на начало которой
   указывает bl. Если указатель bl равен NULL, то будет работать как malloc();
